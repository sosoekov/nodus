...
| `admin`  | всё то же плюс пересчёт раскладки (`POST /api/layout/recompute`)    |
...
Клиенту остаётся показать расхождение и дать переприменить правку. Если записи
...
того, что мешает. Иначе в графе оставались бы висячие рёбра.
